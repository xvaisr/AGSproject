...
+step(D): go(A,B) <- !go_to(A,B).
+step(D): spectaclesAt(X,Y) <- .abolish(spectaclesAt(X,Y)); do(pick);+glass.
+step(D): explore(A,B) <- !go_to(A,B).
...
@discover[atomic]+!discover :  grid_size(MaxX,MaxY) <- 
...
@seen[atomic]+!seen(A,B): not glass<-for(.range(CurrX,A-3,A+3)){
									for(.range(CurrY,B-3,B+3)){
										if(explore(CurrX,CurrY)) {.abolish(explore(CurrX,CurrY))}
									}
								}.
@seen2[atomic]+!seen(A,B): glass<-for(.range(CurrX,A-6,A+6)){
